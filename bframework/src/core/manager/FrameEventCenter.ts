module core {
    ...
    export class FrameEventCenter {
        ...
        private onEnterFrame(event: egret.Event): void {
            ...
            if (callbacks) {
                for (let i: number = 0, iLen: number = callbacks.length; i < iLen; i++) {
                    ...
                    let ret = callback.bindCallback(curTick - this.m_preTick);
                    if (ret) {
                        this.removeFrameEventListeneWithFunction(callback.callback, callback.thisObj);
                    }
                }
            }
         ...
         */
        public addFrameEventListener(callback: (offset: number) => boolean, thisObj: any): void {
            if (callback && thisObj) {
         ...
         * @param  {any} thisObj
         */
        public removeFrameEventListener(callback: (offset: number) => boolean, thisObj: any): void {
         ...
         * @param  {any} thisObj
         */
        private removeFrameEventListeneWithFunction(callback: Function, thisObj: any): void {
            let callbacks: Callback[] = this.m_callbacks;
            if (callbacks) {
                for (let i: number = callbacks.length; i > 0; i--) {
                    let data: Callback = callbacks[i - 1];
