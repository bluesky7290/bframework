module core {
	...
	export class EventManager {
		...
		public sendEvent(message: EventData): void {
			...
			if (message.messageID == ttaby.CustomEventID.GAMEPAUSE || message.messageID == ttaby.CustomEventID.GAMERESUME) {
				this.flush();
			} else {
		...
		private sendAll(): void {
			...
			if (buffLen > buffMax) {
				...
				egret.callLater(this.sendAll, this);
				Log.log(`分帧剩余未处理请求数：${this.m_sendBuffer.length}`);
			}
			...
			if (t_end > allLimit) {
				if (max_data && max > limit) {
					Log.error(`单帧事件派发耗时：${t_end} 最高耗时事件：${max_data.messageID} 耗时：${max}`);
				} else {
					Log.error(`单帧事件派发耗时：${t_end}`);
				}
			}
		}
	}
