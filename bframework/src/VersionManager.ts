class VersionController implements RES.IVersionController {
    // 版本号
    private currentVersion: string = ttaby.CommonFunction.CurrentVersion();
    // 版本控制信息
    ...
    private versionConfigPath = "";
    // 在游戏开始加载资源的时候就会调用这个函数
    init(): Promise<any> {
        if (RELEASE) {//发布模式
            this.resourceRoot = (RELEASE && egret.Capabilities.runtimeType == egret.RuntimeType.WXGAME) ? "https://cdn.h5gamelogin1.haofeigame.com/Buyu/dev/resource/" : "resource/";
            this.versionConfigPath = (RELEASE && egret.Capabilities.runtimeType == egret.RuntimeType.WXGAME) ? "https://cdn.h5gamelogin1.haofeigame.com/Buyu/dev/resource/version" + this.currentVersion + ".json" : "resource/version.json";
